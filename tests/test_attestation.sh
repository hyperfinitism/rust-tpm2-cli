...
run_test "quote" bash -c '
    tpm2 quote -c "file:$TEST_TMPDIR/sign.ctx" -l "sha256:0,1,2" -g sha256 \
...
run_test "quote with qualification" bash -c '
    tpm2 quote -c "file:$TEST_TMPDIR/sign.ctx" -l "sha256:0" -g sha256 \
        -q "hex:deadbeef" \
...
run_test "checkquote" bash -c '
    tpm2 readpublic -c "file:$TEST_TMPDIR/sign.ctx" -o "$TEST_TMPDIR/sign_pub.bin" >/dev/null 2>&1 &&
    tpm2 loadexternal -u "$TEST_TMPDIR/sign_pub.bin" -a n \
        -c "$TEST_TMPDIR/sign_ext.ctx" >/dev/null 2>&1 &&
    tpm2 checkquote \
        -u "file:$TEST_TMPDIR/sign_ext.ctx" \
...
run_test "nvcertify" bash -c '
    ...
    tpm2 nvcertify -C "file:$TEST_TMPDIR/sign.ctx" -i 0x01000020 -c o \
        -P "" -g sha256 \
...
run_test "getcommandauditdigest" bash -c '
    tpm2 getcommandauditdigest -c "file:$TEST_TMPDIR/sign.ctx" -C e \
