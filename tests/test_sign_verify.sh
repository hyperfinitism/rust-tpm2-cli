...
run_test "sign RSA (rsassa)" bash -c '
    tpm2 sign -c "file:$TEST_TMPDIR/sign_rsa.ctx" -g sha256 -s rsassa \
...
run_test "verifysignature RSA" bash -c '
    tpm2 verifysignature -c "file:$TEST_TMPDIR/sign_rsa.ctx" \
...
run_test "sign ECC (ecdsa)" bash -c '
    tpm2 sign -c "file:$TEST_TMPDIR/sign_ecc.ctx" -g sha256 -s ecdsa \
        -d "$TEST_TMPDIR/digest.bin" -t "$TEST_TMPDIR/hash_ticket.bin" \
        -o "$TEST_TMPDIR/sig_ecc.bin" 2>/dev/null &&
    [ -s "$TEST_TMPDIR/sig_ecc.bin" ]
'
run_test "verifysignature ECC" bash -c '
    tpm2 verifysignature -c "file:$TEST_TMPDIR/sign_ecc.ctx" \
...
run_test "verifysignature with external key file" bash -c '
    tpm2 readpublic -c "file:$TEST_TMPDIR/sign_rsa.ctx" -o "$TEST_TMPDIR/sign_rsa_pub.bin" 2>/dev/null &&
...
run_test "verifysignature with -m message" bash -c '
    ...
    tpm2 verifysignature -c "file:$TEST_TMPDIR/sign_rsa2.ctx" -g sha256 \
